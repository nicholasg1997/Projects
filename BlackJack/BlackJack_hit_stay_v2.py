...
class BlackJack(gym.Env):
    ...
    def step(self, action):
        ...
        if done:
            ...
            self.reset()
            return np.array([sum(self.player_hand), self.dealer_hand[0], low, med, high], dtype=np.float32), reward, done, info
...
#model.save('data/PPO_BlackJack_Hit_Stay')


# In[ ]:


action, _ = model.predict(np.array([18, 10]), deterministic=True)
action
...
#model = PPO.load('data/PPO_BlackJack_Hit_Stay') 
